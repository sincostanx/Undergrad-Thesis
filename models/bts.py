...
class upconv(nn.Module):
    def __init__(self, in_channels, out_channels, ratio=2):
        ...
        self.conv = SeperableConv2d(in_channels, out_channels, kernel_size = 3, activation = ["relu", "elu"])
        self.ratio = ratio
...
class bts(nn.Module):
    def __init__(self, feat_out_channels, max_depth, num_features=512):
        ...
        self.conv5      = SeperableConv2d(num_features + feat_out_channels[3], num_features, kernel_size = 3, activation = ["relu", "elu"])
        ...
        self.conv4      = SeperableConv2d(num_features // 2 + feat_out_channels[2], num_features // 2, kernel_size = 3, activation = ["relu", "elu"])
        ...
        self.daspp_conv = SeperableConv2d(num_features + num_features // 2 + num_features // 4, num_features // 4, kernel_size = 3, activation = ["relu", "elu"])
        ...
        self.conv3      = SeperableConv2d(num_features // 4 + feat_out_channels[1] + 1, num_features // 4, kernel_size = 3, activation = ["relu", "elu"])
        ...
        self.conv2      = SeperableConv2d(num_features // 8 + feat_out_channels[0] + 1, num_features // 8, kernel_size = 3, activation = ["relu", "elu"])
        ...
        self.conv1      = SeperableConv2d(num_features // 16 + 4, num_features // 16, kernel_size = 3, activation = ["relu", "elu"])
        self.get_depth  = SeperableConv2d(num_features // 16, 1, kernel_size = 3, activation = ["relu", "sigmoid"])
...
class BtsModel(nn.Module):
    def __init__(self, basemodel_name, bts_size=512, min_val=0.1, max_val=10, norm="linear"):
        super(BtsModel, self).__init__()
        self.min_val = min_val
        self.max_val = max_val
        self.encoder = Encoder(basemodel_name, output_channel=2208)
        self.decoder = bts(self.encoder.feat_out_channels, max_depth=max_val, num_features=bts_size)

    def forward(self, x, focal = torch.rand(1)):
    ...
    @classmethod
    def build(cls, basemodel_name, bts_size, **kwargs):
        m = cls(basemodel_name, bts_size=bts_size, **kwargs)
        return m
