...
class PatchTransformerEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        #print(embeddings.size(),self.positional_encodings[:embeddings.shape[2], :].T.unsqueeze(0).size())
...
class SeperableConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, activation=["none","none"]):
        super(SeperableConv2d, self).__init__()
        self.depthwise = self.depthwiseConv(in_channels, kernel_size, activation[0])
        self.pointwise = self.pointwiseConv(in_channels, out_channels, activation[1])
    
    def getActivation(self, activation):
        if activation=="relu": return nn.ReLU(inplace=True)
        elif activation=="elu": return nn.ELU(inplace=True)
        elif activation=="sigmoid": return nn.Sigmoid()
        elif activation=="leaky_relu": return nn.LeakyReLU()
        elif activation=="softmax2d": return nn.Softmax2d()
    
    def depthwiseConv(self, in_channels, kernel_size, activation = "none"):
        #need to add batchnorm after this
        ...
        #print(activation, self.getActivation(activation))
        if activation != "none": layer.add_module(activation, self.getActivation(activation))
        return layer
    
    def pointwiseConv(self, in_channels, out_channels, activation = "none"):
        layer =  nn.Sequential(
